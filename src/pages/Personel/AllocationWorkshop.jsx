...
import { CreateAllocationworkshop, UpdateAllocationworkshop, GetAllocationworkshop, DeleteAllocationworkshop, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetAllocationworkshop(request)
  debugger
  return {
...
const namespace = {
  grid: 'AllocationworkshopList',
  modal: 'AllocationworkshopModal'
}

class Allocationworkshop extends Component {
  state = {
    modal: false,
    modalRowId: null,
    reloadGrid: false,
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateAllocationworkshop(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateAllocationworkshop(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteAllocationworkshop(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblAllocationworkshop", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblAllocationworkshop", fieldName: "name" },
            { title: 'mineName', showTitle: 'معدن', type: 'string', tableName: "GNR.tblmine", fieldName: "name" },
          ]}
...
mapStore(Allocationworkshop)

export default Allocationworkshop;