...
import { CreateTeamType, UpdateTeamType, DeleteTeamType, GetTeamType, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetTeamType(request)
...
async function loadModal(row) {
    const request = { "id": 43, "actionType": 1 }
...
class TeamType extends Component {
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateTeamType(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateTeamType(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteTeamType(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblTeamType", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblTeamType", fieldName: "name" },
                    ]}
