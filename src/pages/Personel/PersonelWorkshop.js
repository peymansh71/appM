...
import { CreatePersonelWorkshop, UpdatePersonelWorkshop, GetPersonelWorkshop, DeletePersonelWorkshop, CreateForm } from '../../utils/axiosMethod';
// import Swal from 'sweetalert2'
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetPersonelWorkshop(request)
  return {
...
const namespace = {
  grid: 'PersonelWorkshopList',
  modal: 'PersonelWorkshopModal'
}

class PersonelWorkshop extends Component {
  state = {
    modal: false,
    modalRowId: null,
    reloadGrid: false,
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdatePersonelWorkshop(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreatePersonelWorkshop(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeletePersonelWorkshop(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblPersonelWorkshop", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblPersonelWorkshop", fieldName: "name" },
...
mapStore(PersonelWorkshop)

export default PersonelWorkshop;