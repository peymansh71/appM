...
import { CreateWorkshopType, UpdateWorkshopType, DeleteWorkshopType, GetWorkshopType, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetWorkshopType(request)
...
const namespace = {
    grid: 'WorkShopTypeList',
    modal: 'WorkShopTypeModal'
}

class WorkshopType extends Component {
    state = {
        modal: false,
        modalRowId: null,
        reloadGrid: false,
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateWorkshopType(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateWorkshopType(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteWorkshopType(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblworkshopType", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblworkshopType", fieldName: "name" },
                    ]}
