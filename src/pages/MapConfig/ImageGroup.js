...
import { CreateImageGroup, UpdateImageGroup, GetImageGroup, DeleteImageGroup, CreateForm } from '../../utils/axiosMethod';
// import Swal from 'sweetalert2'
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetImageGroup(request)
  debugger
  return {
...
class ImageGroup extends Component {
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateImageGroup(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateImageGroup(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteImageGroup(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblImageGroup", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblImageGroup", fieldName: "name" },
