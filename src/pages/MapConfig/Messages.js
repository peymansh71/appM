...
import { CreateMessages, UpdateMessages, GetMessages, DeleteMessages, CreateForm } from '../../utils/axiosMethod';
// import Swal from 'sweetalert2'
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetMessages(request)
    debugger
    return {
...
class Messages extends Component {
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateMessages(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
                    this.closeModal()
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateMessages(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteMessages(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblMessages", fieldName: "code" },
                        { title: 'description', showTitle: 'شرح پیام', type: 'string', tableName: "GNR.tblMessages", fieldName: "description" },
                    ]}
