...
import { CreateEntityType, UpdateEntityType, GetEntityType, DeleteEntityType, CreateForm } from '../../utils/axiosMethod';
// import Swal from 'sweetalert2'
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  debugger
  const res = await GetEntityType(request)
  debugger
  return {
...
class EntityType extends Component {
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateEntityType(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateEntityType(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteEntityType(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblEntityType", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblEntityType", fieldName: "name" },
            { title: 'borderColor', showTitle: "رنگ حاشیه", type: 'string', tableName: "GNR.tblEntityType", fieldName: "borderColor" },
            { title: 'backgroundColor', showTitle: 'رنگ زمینه', type: 'string', tableName: "GNR.tblEntityType", fieldName: "backgroundColor" },
          ]}
