...
import { CreateBench, UpdateBench, DeleteBench, GetBench, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetBench(request)
    return {
...
class Bench extends Component {
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateBench(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
                    this.closeModal()
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateBench(JSON.parse(JSON.stringify([obj])));
                if (res.status === 200) {
                    this.setState({ reloadGrid: true });
                    this.closeModal()
                    toast.success('با موفیقت انجام شد')
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
          ...
          const res = await DeleteBench(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblbench", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblbench", fieldName: "name" },
                        { title: 'pitName', showTitle: 'پیت', type: 'string', tableName: "GNR.tblpit", fieldName: "name" },
                        { title: 'toe', showTitle: 'ارتفاع پاشنه(متر)', type: 'string', tableName: "GNR.tblbench", fieldName: "toe" },
                        { title: 'crest', showTitle: 'ارتفاع تاج(متر)', type: 'string', tableName: "GNR.tblbench", fieldName: "crest" },
                    ]}
