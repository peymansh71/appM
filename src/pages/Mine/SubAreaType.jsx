...
import { CreateSubAreaType, UpdateSubAreaType, GetSubAreaType, DeleteSubAreaType, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetSubAreaType(request)
  debugger
  return {
...
const namespace = {
  grid: 'SubAreaTypeList',
  modal: 'SubAreaTypeModal'
}

class SubAreaType extends Component {
  state = {
    modal: false,
    modalRowId: null,
    reloadGrid: false,
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateSubAreaType(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateSubAreaType(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    await Swal.fire({
      title: `از حذف ردیف(${index}) مطمئن هستید ؟ `,
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteSubAreaType(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblSubAreaType", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblSubAreaType", fieldName: "name" },
            { title: 'borderColor', showTitle: 'رنگ کادر', type: 'string', tableName: "GNR.tblSubAreaType", fieldName: "borderColor" },
            { title: 'backgroundColor', showTitle: 'زنگ زمینه', type: 'string', tableName: "GNR.tblSubAreaType", fieldName: "backgroundColor" },
          ]}
...
mapStore(SubAreaType)

export default SubAreaType;