...
import { CreatePatern, UpdatePatern, GetPatern, DeletePatern, CreateForm } from '../../utils/axiosMethod';
// import Swal from 'sweetalert2'
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetPatern(request)
  debugger
  return {
...
class Patern extends Component {
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdatePatern(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreatePatern(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeletePatern(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblPatern", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblPatern", fieldName: "name" },
            { title: 'pitName', showTitle: 'پیت', type: 'string', tableName: "GNR.tblpit", fieldName: "name" },
            { title: 'benchName', showTitle: 'پله', type: 'string', tableName: "GNR.tblbench", fieldName: "name" },
            { title: 'zoneName', showTitle: 'ناحیه', type: 'string', tableName: "GNR.tblzone", fieldName: "name" },
            { title: 'startDate', showTitle: 'تاریخ شروع', type: 'string', tableName: "GNR.tblPatern", fieldName: "startDate" },
            { title: 'endDate', showTitle: 'تاریخ پایان', type: 'string', tableName: "GNR.tblPatern", fieldName: "endDate" },
            { title: 'qtyPredictedOre', showTitle: 'مقدار ماده معدنی مورد انتظار', type: 'string', tableName: "GNR.tblPatern", fieldName: "qtyPredictedOre" },
            { title: 'qtyPredictedOreWaste', showTitle: 'مقدار باطله مورد انتظار', type: 'string', tableName: "GNR.tblPatern", fieldName: "qtyPredictedOreWaste" },
            { title: 'oreDensity', showTitle: 'چگالی ماده معدنی', type: 'string', tableName: "GNR.tblPatern", fieldName: "oreDensity" },
            { title: 'wasteDensity', showTitle: 'چگالی باطله', type: 'string', tableName: "GNR.tblPatern", fieldName: "wasteDensity" },
            { title: 'status', showTitle: 'وضعیت', type: 'string', tableName: "GNR.tblPatern", fieldName: "status" },
          ]}
