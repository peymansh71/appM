...
import { CreateSubArea, UpdateSubArea, GetSubArea, DeleteSubArea, CreateForm } from '../../utils/axiosMethod';
// import Swal from 'sweetalert2'
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetSubArea(request)
  return {
...
class SubArea extends Component {
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateSubArea(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateSubArea(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteSubArea(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblSubArea", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblSubArea", fieldName: "name" },
            { title: 'subareaTypeName', showTitle: 'نوع زیر ناحیه', type: 'string', tableName: "GNR.tblSubAreaType", fieldName: "name" },
            { title: 'mineEntityName', showTitle: 'معدن', type: 'string', tableName: "GNR.tblMineEntity", fieldName: "name" },
            { title: 'lithologyName', showTitle: 'سنگ شناسی', type: 'string', tableName: "GNR.tblLithology", fieldName: "name" },
            { title: 'predictedMass', showTitle: 'جرم پیش بینی شده', type: 'string', tableName: "GNR.tblSubArea", fieldName: "predictedMass" },
            { title: 'startDate', showTitle: 'تاریخ شروع به کار', type: 'string', tableName: "GNR.tblSubArea", fieldName: "startDate" },
            { title: 'endDate', showTitle: 'تاریخ اتمام کار', type: 'string', tableName: "GNR.tblSubArea", fieldName: "endDate" },
            { title: 'status', showTitle: 'وضعیت', type: 'string', tableName: "GNR.tblSubArea", fieldName: "status" },
          ]}
