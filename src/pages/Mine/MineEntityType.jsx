...
import { CreateMineEntityType, UpdateMineEntityType, GetMineEntityType, DeleteMineEntityType, CreateForm } from '../../utils/axiosMethod';
// import Swal from 'sweetalert2'
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  debugger
  const res = await GetMineEntityType(request)
...
const namespace = {
  grid: 'MineEntityTypeList',
  modal: 'MineEntityTypeModal'
}

class MineEntityType extends Component {
  state = {
    modal: false,
    modalRowId: null,
    reloadGrid: false,
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateMineEntityType(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateMineEntityType(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteMineEntityType(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblMineEntityType", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblMineEntityType", fieldName: "name" },
...
mapStore(MineEntityType)

export default MineEntityType;