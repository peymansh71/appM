...
import { CreateMineEntity, UpdateMineEntity, GetMineEntity, DeleteMineEntity, CreateForm } from '../../utils/axiosMethod';
// import Swal from 'sweetalert2'
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetMineEntity(request)
    debugger
    return {
...
const namespace = {
    grid: 'MineEntityList',
    modal: 'MineEntityModal'
}

class MineEntity extends Component {
    state = {
        modal: false,
        modalRowId: null,
        reloadGrid: false,
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateMineEntity(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
                    this.closeModal()
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateMineEntity(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteMineEntity(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblMineEntity", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblMineEntity", fieldName: "name" },
                        { title: 'pitName', showTitle: 'پیت', type: 'string', tableName: "GNR.tblpit", fieldName: "name" },
                        { title: 'entityTypeName', showTitle: 'اجزا', type: 'string', tableName: "GNR.tblentityType", fieldName: "name" },
                        { title: 'status', showTitle: 'وضعیت', type: 'string', tableName: "GNR.tblMineEntity", fieldName: "status" },
                    ]}
