...
import { CreateMine, UpdateMine, DeleteMine, GetMine, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetMine(request)
    return {
...
async function loadModal(row) {
    const request = { "id": 29, "actionType": 1 }
...
class Mine extends Component {
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateMine(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateMine(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteMine(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblMine", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblMine", fieldName: "name" },
                    ]}
