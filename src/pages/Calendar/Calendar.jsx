...
import { CreateCalendar, UpdateCalendar, GetCalendar, DeleteCalendar, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetCalendar(request)
  return {
...
class Calendar extends Component {
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateCalendar(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateCalendar(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteCalendar(JSON.parse(JSON.stringify([obj])))
      if (res.status === 200) {
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblCalendar", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblCalendar", fieldName: "name" },
