...
import { CreateCalendarSetting, UpdateCalendarSetting, GetCalendarSetting, DeleteCalendarSetting, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetCalendarSetting(request)
  return {
...
const namespace = {
  grid: 'CalendarSettingList',
  modal: 'CalendarSettingModal'
}
...
class CalendarSetting extends Component {
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateCalendarSetting(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateCalendarSetting(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteCalendarSetting(JSON.parse(JSON.stringify([obj])))
      if (res.status === 200) {
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblCalendarSetting", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblCalendarSetting", fieldName: "name" },
