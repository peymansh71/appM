...
import { CreateLithologyType, UpdateLithologyType, GetLithologyType, DeleteLithologyType, CreateForm } from "../../utils/axiosMethod";
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetLithologyType(request)
    debugger
    return {
...
const namespace = {
    grid: 'LithologhyTypeList',
    modal: 'LithologhyTypeModal'
}

class LithologyType extends Component {
    state = {
        modal: false,
        modalRowId: null,
        reloadGrid: false,
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateLithologyType(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
                    this.closeModal()
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateLithologyType(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteLithologyType(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblLithologyType", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblLithologyType", fieldName: "name" },
                        { title: 'density', showTitle: 'چگالی', type: 'string', tableName: "GNR.tblLithologyType", fieldName: "density" },
                        { title: 'bordercolor', showTitle: 'رنگ حاشیه', type: 'string', tableName: "GNR.tblLithologyType", fieldName: "bordercolor" },
                        { title: 'backgroundcolor', showTitle: 'رنگ زمینه', type: 'string', tableName: "GNR.tblLithologyType", fieldName: "backgroundcolor" },
...
mapStore(LithologyType)

export default LithologyType;