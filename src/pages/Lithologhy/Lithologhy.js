...
import { CreateLithology, UpdateLithology, GetLithology, DeleteLithology, CreateForm } from "../../utils/axiosMethod";
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetLithology(request)
    return {
...
const namespace = {
    grid: 'LithologhyList',
    modal: 'LithologhyModal'
}

class Lithology extends Component {
    state = {
        modal: false,
        modalRowId: null,
        reloadGrid: false,
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateLithology(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
                    this.closeModal()
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateLithology(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteLithology(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblLithology", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblLithology", fieldName: "name" },
                        { title: 'lithologyTypeName', showTitle: 'نوع سنگ', type: 'string', tableName: "GNR.tblLithologyType", fieldName: "name" },
                        { title: 'description', showTitle: 'توضیحات', type: 'string', tableName: "GNR.tblLithology", fieldName: "description" },
                        { title: 'abbreviation', showTitle: 'نام اختصاری', type: 'string', tableName: "GNR.tblLithology", fieldName: "abbreviation" },
                        { title: 'density', showTitle: 'چگالی', type: 'string', tableName: "GNR.tblLithology", fieldName: "density" },
                        { title: 'borderColorName', showTitle: 'رنگ کادر', type: 'string', tableName: "GNR.tblLithology", fieldName: "borderColorName" },
                        { title: 'backgroundColorName', showTitle: 'زنگ زمینه', type: 'string', tableName: "GNR.tblLithology", fieldName: "backgroundColorName" },
                    ]}
