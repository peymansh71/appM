...
import { CreateChangeSerial, UpdateChangeSerial, GetChangeSerial, DeleteChangeSerial, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetChangeSerial(request)
    return {
...
const namespace = {
    grid: 'ChangeSerialList',
    modal: 'ChangeSerialModal'
}

class ChangeSerial extends Component {
    state = {
        modal: false,
        modalRowId: null,
        reloadGrid: false,
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateChangeSerial(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateChangeSerial(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteChangeSerial(JSON.parse(JSON.stringify([obj])))
            if (res.status === 200) {
            ...
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        ...
                        { title: 'equipmentName', showTitle: 'تجهیز', type: 'string', tableName: "GNR.tblequipment", fieldName: "name" },
                        { title: 'newSerial', showTitle: 'سریال ', type: 'string', tableName: "GNR.tblChangeSerial", fieldName: "newSerial" },
                    ]}
...
mapStore(ChangeSerial)

export default ChangeSerial;