...
import { CreateTab, UpdateTab, DeleteTab, GetTab, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetTab(request)
    debugger
    return {
...
async function loadModal(row) {
    const request = { "id": 41, "actionType": 1 }
...
class Tab extends Component {
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateTab(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateTab(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteTab(JSON.parse(JSON.stringify([obj])))
            if (res.status === 200) {
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblTab", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblTab", fieldName: "name" },
                        { title: 'equipmenttypeName', showTitle: 'نوع تجهیز ', type: 'string', tableName: "GNR.tblEquipmentType", fieldName: "name" },
                        { title: 'tabPositionName', showTitle: 'موقعیت', type: 'string', tableName: "GNR.tblTab", fieldName: "name" },
                    ]}
