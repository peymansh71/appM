...
import { CreateGroup, UpdateGroup, GetGroup, DeleteGroup, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetGroup(request)
    return {
...
class Group extends Component {
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateGroup(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
                    this.closeModal()
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateGroup(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteGroup(JSON.parse(JSON.stringify([obj])))
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblGroup", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblGroup", fieldName: "name" }
                    ]}
