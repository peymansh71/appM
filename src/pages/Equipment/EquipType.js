...
import { CreateEquipType, UpdateEquipType, DeleteEquipType, CreateForm, GetEquipType } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetEquipType(request)
  return {
...
async function loadModal(row) {
  const request = { "id": 21, "actionType": 1 }
...
const namespace = {
  grid: 'equipmentTypeList',
  modal: 'equipmentTypeModal'
}

class EquipType extends Component {
  state = {
    modal: false,
    modalRowId: null,
    reloadGrid: false,
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateEquipType(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateEquipType(JSON.parse(JSON.stringify([obj])));
        if (res.status === 200) {
          this.setState({ reloadGrid: true });
          this.closeModal()
          // const { setStore, getStore } = this.context;
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteEquipType(JSON.parse(JSON.stringify([obj])))
      if (res.status === 200) {
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblequipmentType", fieldName: "code" },
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblequipmentType", fieldName: "name" },
          ]}
