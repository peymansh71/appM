...
import { CreateStateGroup, UpdateStateGroup, GetStateGroup, DeleteStateGroup, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
    ...
    const res = await GetStateGroup(request)
    debugger
    return {
...
async function loadModal(row) {
    const request = { "id": 38, "actionType": 1 }
...
class StateGroup extends Component {
    ...
    saveModal = async (data) => {
        ...
        if (data.id) {
            try {
                const res = await UpdateStateGroup(JSON.parse(JSON.stringify([obj])))
                if (res.status === 200) {
        ...
        //add
            ...
            // setStore('grid', store.grid)
            try {
                const res = await CreateStateGroup(JSON.parse(JSON.stringify([obj])));
    ...
    removeGridRow = async (row, index) => {
        ...
        if (this.state.isDelete) {
            ...
            const res = await DeleteStateGroup(JSON.parse(JSON.stringify([obj])))
            if (res.status === 200) {
        ...
        return (
            <Masterpage>
                ...
                <Grid
                    ...
                    columns={[
                        { title: 'code', showTitle: 'کد', type: 'string', tableName: "GNR.tblstategroup", fieldName: "code" },
                        { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNR.tblstategroup", fieldName: "name" },
                        { title: 'equipmentTypeName', showTitle: 'نوع تجهیز', type: 'string', tableName: "GNR.tblequipmentType", fieldName: "name" },
                        { title: 'stateName', showTitle: 'وضعیت ', type: 'string', tableName: "GNR.tblstate", fieldName: "name" },
                    ]}
