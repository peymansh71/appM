...
import { CreateNumberOfShovelfuls, UpdateNumberOfShovelfuls, GetNumberOfShovelfuls, DeleteNumberOfShovelfuls, CreateForm } from '../../utils/axiosMethod';
...
async function loadGird({ columns, pageSize, pageNumber }) {
  ...
  const res = await GetNumberOfShovelfuls(request)
  return {
...
const namespace = {
  grid: 'NumberOfShovelfulsList',
  modal: 'NumberOfShovelfulsModal'
}

class NumberOfShovelfuls extends Component {
  state = {
    modal: false,
    modalRowId: null,
    reloadGrid: false,
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateNumberOfShovelfuls(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateNumberOfShovelfuls(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteNumberOfShovelfuls(JSON.parse(JSON.stringify([obj])))
      if (res.status === 200) {
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'Loadingequipment', showTitle: 'تجهیز بارگیری', type: 'string', tableName: "GNR.tblequipment", fieldName: "Loadingequipment" },
            { title: 'Haulingequipment', showTitle: 'تجهیز باربری', type: 'string', tableName: "GNR.tblequipment", fieldName: "Haulingequipment" },
            { title: 'Numberofshovelfuls', showTitle: 'تعداد بیل پرکردن ', type: 'string', tableName: "GNR.tblNumberOfShovelfuls", fieldName: "Numberofshovelfuls" },
            { title: 'Totaltimeoflosding', showTitle: 'زمان پرشدن ماشین بار بر', type: 'string', tableName: "GNR.tblNumberOfShovelfuls", fieldName: "Totaltimeoflosding" },
          ]}
...
mapStore(NumberOfShovelfuls)

export default NumberOfShovelfuls;