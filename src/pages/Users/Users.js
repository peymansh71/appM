...
import { CreateUsers, UpdateUsers, GetUsers, DeleteUsers, CreateForm } from '../../utils/axiosMethod';
...
class Users extends Component {
  ...
  saveModal = async (data) => {
    ...
    if (data.id) {
      try {
        const res = await UpdateUsers(JSON.parse(JSON.stringify([obj])))
        if (res.status === 200) {
          this.closeModal()
    ...
    //add
      ...
      // setStore('grid', store.grid)
      try {
        const res = await CreateUsers(JSON.parse(JSON.stringify([obj])));
  ...
  removeGridRow = async (row, index) => {
    ...
    if (this.state.isDelete) {
      ...
      const res = await DeleteUsers(JSON.parse(JSON.stringify([obj])))
    ...
    return (
      <Masterpage>
        ...
        <Grid
          ...
          columns={[
            { title: 'name', showTitle: 'نام', type: 'string', tableName: "GNuser", fieldName: "name" },
            { title: 'family', showTitle: 'نام خانوادگی', type: 'string', tableName: "GNR.tbluser", fieldName: "family" },
            { title: 'userName', showTitle: 'نام کاربری', type: 'string', tableName: "GNR.tbluser", fieldName: "userName" },
            { title: 'personelCode', showTitle: 'کد پرسنلی', type: 'string', tableName: "GNR.tblpersonel", fieldName: "code" },
            { title: 'isAdmin', showTitle: 'دسترسی ادمین', type: 'string', tableName: "GNR.tbluser", fieldName: "code" },
            { title: 'isActive', showTitle: 'وضعیت', type: 'string', tableName: "GNR.tbluser", fieldName: "code" },
          ]}
